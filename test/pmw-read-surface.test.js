...
test('buildPmwReadSurface assembles overview, cards, and artifacts from repo-local sources', () => {
  ...
  assert.equal(surface.artifacts.byKey['requirements'].path, 'REQUIREMENTS.md');
  assert.equal(surface.artifacts.byKey['latest-handoff'].summary, 'DEV-04 mockup approved, code implementation may start.');
  assert.equal(surface.artifacts.byKey['dev04-packet'].summary, 'PMW first view를 overview, 4-card grid, artifact preview로 읽게 만든다.');
});

test('buildPmwReadSurface prefers standard-template artifact paths when available', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'pmw-surface-template-'));
  seedRepo(repoRoot);
  seedTemplateOverlay(repoRoot);
  const store = createOperatingStateStore({
    dbPath: path.join(repoRoot, '.harness', 'operating_state.sqlite'),
    now: createClock('2026-04-19T10:30:00.000Z')
  });

  store.setReleaseState({
    currentStage: 'implementation',
    releaseGateState: 'open',
    currentFocus: 'DEV-05 validator lane',
    releaseGoal: 'Apply standard template to the current project harness',
  ...
  store.upsertWorkItem({
    ...
    status: 'in_progress',
    nextAction: 'Keep template overlay paths active',
    domainHint: '검증·컷오버',
  ...
  assert.equal(surface.overview.views.purpose.docs.includes('.agents/artifacts/REQUIREMENTS.md'), true);
});

test('createPmwServer serves HTML and JSON read surface responses', async () => {
...
function seedRepo(repoRoot) {
  fs.mkdirSync(path.join(repoRoot, 'codex', 'project-context'), { recursive: true });
  ...
  fs.writeFileSync(path.join(repoRoot, 'UI_DESIGN.md'), '# UI Design\n\n## Summary\ncompact 4-card rail과 lower document preview를 유지한다.\n', 'utf8');
  fs.writeFileSync(path.join(repoRoot, 'codex', 'project-context', 'active-state.md'), '# Active State\n\n## Summary\nDEV-04 implementation in progress.\n', 'utf8');
  fs.writeFileSync(path.join(repoRoot, 'codex', 'project-context', 'preventive-memory.md'), '# Preventive Memory\n\n- keep strong surface on approved source only\n', 'utf8');
}

function seedTemplateOverlay(repoRoot) {
