...
const FILES = {
  requirements: ARTIFACT_PATHS.requirements,
  architecture: ARTIFACT_PATHS.architecture,
  plan: ARTIFACT_PATHS.plan,
  progress: ARTIFACT_PATHS.progress,
  ui: ARTIFACT_PATHS.ui,
  workItemPacketTemplate: "reference/packets/PKT-01_WORK_ITEM_PACKET_TEMPLATE.md",
  deploymentPlan: "reference/artifacts/DEPLOYMENT_PLAN.md",
  packetExitQualityGate: "reference/artifacts/PACKET_EXIT_QUALITY_GATE.md",
  pmwPacket: ARTIFACT_PATHS.pmwPacket,
  dev05Packet: ARTIFACT_PATHS.dev05Packet,
  reviewReport: ARTIFACT_PATHS.reviewReport,
  cutoverReport: CUTOVER_REPORT_MARKDOWN,
  active: ARTIFACT_PATHS.active,
  preventive: ARTIFACT_PATHS.preventive
};
...
export function buildPmwReadSurface({ store, repoRoot = process.cwd(), outputDir = repoRoot, dbPath, verificationLane = "npm.cmd test", readModel }) {
  ...
  const evidence = buildLaneEvidence({ current, model, root, out });
  const progress = buildProgress(workItems, model, progressText);
  const artifacts = buildArtifacts({ root, out, model, evidence });
  ...
  return {
    title: resolveSurfaceTitle(model, store),
    ...
    overview: {
      ...
      views: {
        purpose: {
          label: "프로젝트 목적",
          leftLabel: "사용자 목표",
          left: goalLines(req, ["### 사용자 목표", "### 추진목적"]),
          rightLabel: "운영 / 승인 목표",
          right: [
            ...goalLines(req, ["### 운영 목표", "### 기대효과"]),
            ...goalLines(req, ["### 승인 목표"])
          ],
        ...
        approach: {
          ...
          rightLabel: "승인 · 운영 규칙",
          right: evidence.approachLines,
          docs: presentPaths(root, out, [FILES.plan, FILES.ui, ...evidence.approachDocs])
        },
        progress: {
          label: "프로젝트 진행 현황",
          ...
          tableLabel: "전체 작업표",
          rows: progress.rows,
          docs: presentPaths(root, out, [FILES.progress, CURRENT_STATE_DOC, TASK_LIST_DOC, ...evidence.progressDocs])
        }
      }
    },
    cards: [
...
<section class="panel section"><div class="head"><div><div class="eyebrow">Project Overview</div><h2>프로젝트 개요</h2></div><button id="toggle" class="btn${surface.overview.collapsedByDefault ? "" : " active"}" type="button">${surface.overview.collapsedByDefault ? "개요 펼치기" : "개요 접기"}</button></div><div id="overview-shell"${surface.overview.collapsedByDefault ? ' class="hidden"' : ""}><div id="nav" class="nav"></div><div id="overview"></div></div></section>
...
let overviewCollapsed = Boolean(s.overview.collapsedByDefault);
...
function syncOverviewToggle(){ const box = q("#overview-shell"); const btn = q("#toggle"); box.classList.toggle("hidden", overviewCollapsed); btn.textContent = overviewCollapsed ? "개요 펼치기" : "개요 접기"; btn.classList.toggle("active", !overviewCollapsed); }
q("#refresh").onclick = ()=>location.reload(); q("#toggle").onclick = ()=>{ overviewCollapsed = !overviewCollapsed; syncOverviewToggle(); };
renderMeta(); renderOverview(); syncOverviewToggle(); renderCards(); renderArtifacts(); renderArtifact(); renderSettings();
</script></body></html>`;
}
...
function buildArtifacts({ root, out, model, evidence }) {
  const live = [
  ...
  const contracts = [
    ...
    ...evidence.contractArtifacts.map((item) =>
      fileArtifact(item.key, item.fallbackTitle, "주요 계약 문서", item.path, root, out)
    )
  ].filter(Boolean);
...
function buildLaneEvidence({ current, model, root, out }) {
  const lane = current?.workItemId ?? detectLane(model.releaseState.currentFocus);

  if (lane === "REV-01" || lane === "SEC-01") {
    const reviewReport = read(resolve(FILES.reviewReport, root, out));
    const releaseRef = firstExistingRef(root, out, [
      FILES.dev05Packet,
      FILES.packetExitQualityGate,
      FILES.deploymentPlan
    ]);
    return {
      approachLines: latestTopLevelBullets(reviewReport),
      approachDocs: compactRefs([FILES.reviewReport, FILES.cutoverReport, releaseRef]),
      progressDocs: compactRefs([FILES.reviewReport, FILES.cutoverReport, releaseRef]),
      contractArtifacts: [
        { key: "review-report", fallbackTitle: "Review Report", path: FILES.reviewReport },
        { key: "cutover-precheck", fallbackTitle: "Cutover Precheck", path: FILES.cutoverReport },
        buildReleaseArtifactDescriptor(releaseRef)
      ]
    };
  }
  ...
  if (lane === "DEV-05") {
    ...
    return {
      approachLines: readApprovalSurfaceLines(releaseRef, root, out),
      approachDocs: compactRefs([releaseRef]),
      progressDocs: compactRefs([releaseRef, FILES.cutoverReport]),
      contractArtifacts: [
        buildReleaseArtifactDescriptor(releaseRef),
        { key: "cutover-precheck", fallbackTitle: "Cutover Precheck", path: FILES.cutoverReport }
      ]
  ...
  const packetRef = firstExistingRef(root, out, [FILES.pmwPacket, FILES.workItemPacketTemplate]);
  return {
    approachLines: readApprovalSurfaceLines(packetRef, root, out),
    approachDocs: compactRefs([packetRef]),
    progressDocs: compactRefs([packetRef]),
    contractArtifacts: [
      buildPacketArtifactDescriptor(packetRef)
    ]
  };
}
...
function resolve(rel, root, out) {
  ...
  return key && Object.hasOwn(ARTIFACT_PATHS, key) ? resolveArtifactPath(root, key) : path.resolve(root, rel);
}
...
function displayPath(rel, root, out) {
  ...
  const key = FILE_KEYS.get(rel);
  return key && Object.hasOwn(ARTIFACT_PATHS, key) ? resolveArtifactRelativePath(root, key) : rel;
}
...
function firstExistingRef(root, out, refs) {
  return refs.find((ref) => ref && exists(ref, root, out)) ?? refs.find(Boolean) ?? null;
}
...
function readApprovalSurfaceLines(ref, root, out) {
  if (!ref) {
    return ["approval surface가 아직 지정되지 않았다."];
  }
  const text = read(resolve(ref, root, out));
  const lines = bullets(section(text, "## Approval Rule"));
  return lines.length ? lines : preview(text);
}

function buildPacketArtifactDescriptor(ref) {
  if (ref === FILES.workItemPacketTemplate) {
    return { key: "packet-template", fallbackTitle: "Work Item Packet Template", path: ref };
  }
  return { key: "dev04-packet", fallbackTitle: "DEV-04 Packet", path: ref ?? FILES.workItemPacketTemplate };
}

function buildReleaseArtifactDescriptor(ref) {
  if (ref === FILES.packetExitQualityGate) {
    return { key: "packet-exit-quality-gate", fallbackTitle: "Packet Exit Quality Gate", path: ref };
  }
  if (ref === FILES.deploymentPlan) {
    return { key: "deployment-plan", fallbackTitle: "Deployment Plan", path: ref };
  }
  return { key: "dev05-packet", fallbackTitle: "DEV-05 Packet", path: ref ?? FILES.deploymentPlan };
}

function detectLane(currentFocus) {
  const match = String(currentFocus ?? "").match(/\b[A-Z]{3}-\d{2}\b/);
...
function latestTopLevelBullets(text) {
  const normalized = (text ?? "").replace(/\r\n/g, "\n");
  const headings = [...normalized.matchAll(/^##\s+.+$/gm)].map((match) => match[0].trim());
  if (headings.length === 0) {
    return preview(text);
  }
  const lines = bullets(section(normalized, headings.at(-1)));
  return lines.length ? lines : preview(text);
}
...
function goalLines(text, headings) {
  for (const heading of headings) {
    const lines = ordered(section(text, heading)).concat(bullets(section(text, heading)));
    if (lines.length) {
      return lines;
    }
  }
...
function resolveSurfaceTitle(model, store) {
  const releaseState = typeof store?.getReleaseState === "function" ? store.getReleaseState("current") : null;
  const projectName = model.releaseState.metadata?.projectName ?? releaseState?.metadata?.projectName;
  return projectName && String(projectName).trim() ? String(projectName).trim() : "Project Monitor Workspace";
}
