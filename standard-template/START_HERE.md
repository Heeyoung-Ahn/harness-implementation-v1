...
5. 터미널 질문에 답한다.
6. 초기화가 끝나면 터미널에 `Standard harness starter initialized.`와 함께 프로젝트 이름, active profiles, 다음 행동이 출력된다. `INIT_STANDARD_HARNESS.cmd`를 더블클릭으로 실행했다면 요약을 읽을 수 있게 창이 바로 닫히지 않고 대기해야 한다.
...
1. `npm run harness:status`로 kickoff 상태가 열렸는지 확인한다.
2. `reference/artifacts/PROJECT_STARTER_DOC_PACK.md`를 rough baseline으로 먼저 채운다.
3. 그 내용을 바탕으로 `reference/planning/PLN-00_DEEP_INTERVIEW.md`와 `.agents/artifacts/REQUIREMENTS.md`를 실제로 업데이트한다.
4. `reference/planning/PLN-01_REQUIREMENTS_FREEZE.md`에 approved / deferred / open 상태를 남긴다.
5. `PLN-01`이 충분히 승인되기 전까지는 `.agents/artifacts/ARCHITECTURE_GUIDE.md`, `.agents/artifacts/IMPLEMENTATION_PLAN.md`, `reference/artifacts/UI_DESIGN.md`, 첫 task packet을 새 기준선으로 올리지 않는다.

그 다음에 AI에게 `프로젝트 시작용 deep interview`를 요청한다.
