...
## Summary
아키텍처는 `DB truth + Markdown truth + generated docs + read-only PMW`의 4층 runtime 구조를 기본값으로 유지한다. 이 runtime 구조는 `core / optional profile / project packet` 3층 activation 모델 위에서 동작한다.
