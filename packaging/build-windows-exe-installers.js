...
function harnessInstallCmd() {
  return [
    ...
    "\"%~dp0node.exe\" \"%WORK%\\installer\\install-harness.js\" --authority-source local %*",
...
function harnessInstallUiPowerShell() {
  return [
    ...
    "  & $NodePath (Join-Path $work 'installer\\install-harness.js') --non-interactive --authority-source local --project-name $projectName --target-dir $targetDir --profiles $profiles",
