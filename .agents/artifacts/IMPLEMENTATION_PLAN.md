...
- PLN-00 deep interview using `reference/planning/PLN-00_DEEP_INTERVIEW.md` until implementation-critical issues close
...
## PLN-00 Execution
### Goal
`.agents/artifacts/REQUIREMENTS.md`의 open question을 baseline decision으로 닫아 downstream 문서와 구현이 같은 가정을 공유하게 만든다.

### Input
- `.agents/artifacts/REQUIREMENTS.md` open questions
- `PROTOTYPE_REFERENCE.md` carry-forward contract
- `HARNESS_FULL_DESIGN_REVIEW.md` first-ship direction
- `reference/planning/PLN-00_DEEP_INTERVIEW.md` recommended baseline and closure format
...
### Input
- `.agents/artifacts/REQUIREMENTS.md` frozen baseline
- `.agents/artifacts/ARCHITECTURE_GUIDE.md` synced architecture contract
- `reference/artifacts/UI_DESIGN.md` rough/detailed design rule
- `reference/packets/PKT-01_WORK_ITEM_PACKET_TEMPLATE.md` task-level packet template
...
- task-level packet은 기본적으로 `reference/packets/PKT-01_WORK_ITEM_PACKET_TEMPLATE.md` 형식을 따른다.
...
2. `reference/packets/PKT-01_WORK_ITEM_PACKET_TEMPLATE.md`를 사용해 task-level packet으로 목표, 비범위, 상세 동작, 상태/화면 변화, edge case, acceptance를 구체화한다.
