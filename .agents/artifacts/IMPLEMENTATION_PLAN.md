...
5. `REV-03`로 simulation-derived remediation이 실제 generalized project kickoff 전 필요한 reusable gap을 닫았는지 최종 검토한다.
...
### Input
- `reference/artifacts/STANDARD_HARNESS_PROJECT_SIMULATION_REPORT.md`
- `.agents/artifacts/REQUIREMENTS.md`
- `.agents/artifacts/ARCHITECTURE_GUIDE.md`
- `reference/packets/PKT-01_WORK_ITEM_PACKET_TEMPLATE.md`
...
### Exit Criteria
- generalized project work item이 `PRF-01 + PRF-02` 조합을 억지 packet split 없이 표현할 수 있다.
...
- domain foundation reference는 `.agents/artifacts/DOMAIN_CONTEXT.md` 또는 승인된 동등 artifact를 사용하고, 핵심 엔티티/관계, lifecycle state/invariant, source-of-truth, schema impact, migration/rollback boundary, open question을 포함해야 한다.
