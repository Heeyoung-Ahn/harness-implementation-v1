...
- Preserve the approved release-ready standardized harness baseline as the last known good snapshot.
- Keep the generalized follow-up baseline and the closed `SIM-01` / `SIM-02` / `SIM-03` / `REV-03` outputs intact as the current reusable standard.
- Preserve the approved generalized baseline and the now real-world-ready `standard-template/` until a new approved lane opens.
- Keep `standard-template/` synchronized with future reusable standard-harness changes.
- Reopen a narrow starter-hardening lane if launcher/runtime preflight, shipped tests, starter review/test templates, or copied-starter usability drift again.
- Register and keep concrete active packets aligned in `artifact_index` whenever a new lane opens so validator-backed packet enforcement remains trustworthy.
